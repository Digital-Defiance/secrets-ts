...
describe("Secrets Performance Tests", function(): void {
    ...
    describe("Performance Test Infrastructure", function(): void {
        
        it("should have performance helper available", function(): void {
        ...
        it("should measure execution time accurately", function(): void {
            ...
            let measurement = PerformanceHelper.measureExecution(testFn)
        ...
        it("should run benchmarks with multiple iterations", function(): void {
            let testFn = function(): void {
                return Math.random()
            }
            
            let result: string = PerformanceHelper.benchmark("Test benchmark", testFn, {
                iterations: 10,
                warmupIterations: 2
            })
    ...
    describe("Share Generation Performance", function(): void {
        
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark share generation with small secrets (64-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(64, 64)
            
            let testFn = function(): void {
                return secrets.share(secret, 5, 3)
            }
            
            let result: string = PerformanceHelper.benchmark("Share generation (64-bit)", testFn, {
                iterations: 100
            })
        ...
        it("should benchmark share generation with medium secrets (256-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(256, 256)
            
            let testFn = function(): void {
                return secrets.share(secret, 5, 3)
            }
            
            let result: string = PerformanceHelper.benchmark("Share generation (256-bit)", testFn, {
                iterations: 100
            })
        ...
        // Requirement 5.2: Performance with large secrets (up to 1024 bits)
        it("should benchmark share generation with large secrets (512-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(512, 512)
            
            let testFn = function(): void {
                return secrets.share(secret, 5, 3)
            }
            
            let result: string = PerformanceHelper.benchmark("Share generation (512-bit)", testFn, {
                iterations: 50
            })
    ...
    describe("Share Combination Performance", function(): void {
        
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark share combination with small secrets (64-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(64, 64)
            let shares: string[] = secrets.share(secret, 5, 3)
            let thresholdShares = shares.slice(0, 3)
            
            let testFn = function(): void {
                return secrets.combine(thresholdShares)
            }
            
            let result: string = PerformanceHelper.benchmark("Share combination (64-bit)", testFn, {
                iterations: 100
            })
        ...
        it("should benchmark share combination with medium secrets (256-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(256, 256)
            let shares: string[] = secrets.share(secret, 5, 3)
            let thresholdShares = shares.slice(0, 3)
            ...
            let result: string = PerformanceHelper.benchmark("Share combination (256-bit)", testFn, {
                iterations: 100
            })
        ...
        // Requirement 5.2: Performance with large secrets
        it("should benchmark share combination with large secrets (512-bit)", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(512, 512)
            let shares: string[] = secrets.share(secret, 5, 3)
            let thresholdShares = shares.slice(0, 3)
            ...
            let result: string = PerformanceHelper.benchmark("Share combination (512-bit)", testFn, {
                iterations: 50
            })
    ...
    describe("Maximum Share Count Performance", function(): void {
        
        // Requirement 5.3: Performance with maximum shares (255)
        it("should handle maximum share count with 8-bit configuration", function(): void {
            ...
            let result: string = PerformanceHelper.benchmark("Share generation (255 shares)", testFn, {
                iterations: 10
            })
        ...
        it("should combine shares efficiently with many shares", function(): void {
            secrets.init(8)
            let secret: string = TestDataGenerators.generateRandomSecret(64, 64)
            let shares: string[] = secrets.share(secret, 50, 25)
            let thresholdShares = shares.slice(0, 25)
            
            let testFn = function(): void {
                return secrets.combine(thresholdShares)
            }
            
            let result: string = PerformanceHelper.benchmark("Share combination (25 of 50 shares)", testFn, {
                iterations: 20
            })
    ...
    describe("String Conversion Performance", function(): void {
        
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark str2hex conversion", function(): void {
            let testString = TestDataGenerators.generateRandomString(50, 100)
            
            let testFn = function(): void {
                return secrets.str2hex(testString)
            }
            
            let result: string = PerformanceHelper.benchmark("str2hex conversion", testFn, {
                iterations: 100
            })
        ...
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark hex2str conversion", function(): void {
            let testString = TestDataGenerators.generateRandomString(50, 100)
            let hexString = secrets.str2hex(testString)
            
            let testFn = function(): void {
                return secrets.hex2str(hexString)
            }
            
            let result: string = PerformanceHelper.benchmark("hex2str conversion", testFn, {
                iterations: 100
            })
    ...
    describe("NewShare Generation Performance", function(): void {
        
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark newShare generation", function(): void {
            let secret: string = TestDataGenerators.generateRandomSecret(128, 128)
            let shares: string[] = secrets.share(secret, 10, 5)
            let thresholdShares = shares.slice(0, 5)
            
            let testFn = function(): void {
                return secrets.newShare(11, thresholdShares)
            }
            
            let result: string = PerformanceHelper.benchmark("newShare generation", testFn, {
                iterations: 100
            })
    ...
    describe("Random Number Generation Performance", function(): void {
        
        // Requirement 5.1: Performance benchmarks for major operations
        it("should benchmark random number generation", function(): void {
            let testFn = function(): void {
                return secrets.random(256)
            }
            
            let result: string = PerformanceHelper.benchmark("Random number generation (256-bit)", testFn, {
                iterations: 100
            })
    ...
    describe("Performance Consistency", function(): void {
        ...
        it("should have consistent performance across multiple runs", function(): void {
            ...
            for (let i: number = 0; i < runs; i++) {
                ...
                let result: string = PerformanceHelper.benchmark("Consistency test run " + (i + 1), testFn, {
                    iterations: 20
                })
            ...
            let mean = results.reduce(function(sum, val) { return sum + val }, 0) / runs
            let variance = results.reduce(function(sum, val) { return sum + Math.pow(val - mean, 2) }, 0) / runs
    ...
    describe("Memory Usage Tests", function(): void {
        ...
        it("should not leak memory during repeated share generation", function(): void {
            ...
            let result: string = PerformanceHelper.memoryStressTest(function(): void {
                return secrets.share(secret, 10, 5)
            }, iterations)
            
            // Memory growth should be reasonable (less than 10MB for 1000 iterations)
            let memoryGrowthMB = result.memoryGrowth / (1024 * 1024)
        ...
        it("should not leak memory during repeated share combination", function(): void {
            if (typeof process === "undefined" || !process.memoryUsage) {
                pending("Memory tests only run in Node.js environment")
                return
            }
        ...
        it("should handle large secrets without excessive memory usage", function(): void {
            if (typeof process === "undefined" || !process.memoryUsage) {
                pending("Memory tests only run in Node.js environment")
                return
            }
            ...
            let measurement = PerformanceHelper.measureExecution(function(): void {
                let shares: string[] = secrets.share(secret, 10, 5)
                return secrets.combine(shares.slice(0, 5))
            })
            
            // Memory used should be reasonable (less than 50MB for a single large operation)
            let memoryUsedMB = Math.abs(measurement.memoryUsed) / (1024 * 1024)
    ...
    describe("Stress Tests", function(): void {
        ...
        it("should handle stress test with many operations", function(): void {
            let operations = 100
            let maxFailures = 5 // Allow up to 5% failure rate
            let failures = 0
            
            for (let i: number = 0; i < operations; i++) {
                try {
                    let secret: string = TestDataGenerators.generateRandomSecret(64, 256)
        ...
        it("should handle stress test with high share counts", function(): void {
            ...
            for (let i: number = 0; i < operations; i++) {
                try {
                    ...
                    let numShares: number = Math.floor(Math.random() * 50) + 20 // 20-70 shares
                    let threshold: number = Math.floor(numShares / 2) + 1
        ...
        it("should handle stress test with large bit sizes", function(): void {
            let operations = 20
            let failures = 0
            
            for (let i: number = 0; i < operations; i++) {
                try {
                    let bitSize = Math.floor(Math.random() * 512) + 256 // 256-768 bits
                    let secret: string = TestDataGenerators.generateRandomSecret(bitSize, bitSize)
    ...
    describe("Property 7: Performance Bounds", function(): void {
        ...
        it("should complete share generation within time bounds for random inputs", function(): void {
            let iterations = 100
            let maxDuration = 1000 // 1 second max for any single operation
            let failures = []
            
            for (let i: number = 0; i < iterations; i++) {
                // Generate random test data with varying sizes
                let bitSize = Math.floor(Math.random() * 504) + 8 // 8 to 512 bits
                let secret: string = TestDataGenerators.generateRandomSecret(bitSize, bitSize)
                let config: SecretsConfig = TestDataGenerators.generateShareConfig(50) // Up to 50 shares
        ...
        it("should complete share combination within time bounds for random inputs", function(): void {
            let iterations = 100
            let maxDuration = 1000 // 1 second max for any single operation
            let failures = []
        ...
        it("should complete newShare generation within time bounds for random inputs", function(): void {
            let iterations = 100
            let maxDuration = 1000 // 1 second max for any single operation
            let failures = []
        ...
        it("should complete string conversion within time bounds for random inputs", function(): void {
            let iterations = 100
            let maxDuration = 100 // 100ms max for string conversions
            let failures = []
            
            for (let i: number = 0; i < iterations; i++) {
                // Generate random strings of varying lengths
                let str: string = TestDataGenerators.generateRandomString(10, 200)
                
                let measurement = PerformanceHelper.measureExecution(function(): void {
                    let hex: string = secrets.str2hex(str)
        ...
        it("should handle large inputs (up to 1024 bits) within reasonable time", function(): void {
            let iterations = 20 // Fewer iterations for large inputs
            let maxDuration = 5000 // 5 seconds max for very large operations
            let failures = []
            
            for (let i: number = 0; i < iterations; i++) {
                // Test with large bit sizes (512-1024 bits)
                let bitSize = Math.floor(Math.random() * 512) + 512 // 512 to 1024 bits
                let secret: string = TestDataGenerators.generateRandomSecret(bitSize, bitSize)
                let config: SecretsConfig = { numShares: 10, threshold: 5 } // Reasonable share count
        ...
        it("should handle maximum share counts within reasonable time", function(): void {
            secrets.init(8) // 8-bit allows up to 255 shares
            let iterations = 10 // Fewer iterations for max shares
            let maxDuration = 10000 // 10 seconds max for maximum share operations
            let failures = []
            
            for (let i: number = 0; i < iterations; i++) {
                let secret: string = TestDataGenerators.generateRandomSecret(64, 128)
                let numShares: number = Math.floor(Math.random() * 155) + 100 // 100 to 255 shares
                let threshold: number = Math.floor(numShares / 2) + 1
